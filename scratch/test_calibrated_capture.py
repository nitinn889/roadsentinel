...
print("=" * 60)
print(">>> TEST CALIBRATED SCENECAPTURE2D <<<")
print("=" * 60)
...
# Let us test 4 lighting intensity scalings with post_process_blend_weight = 1.0 (default tone curve):
# sun_intensity = 3.0, 5.0, 10.0, 20.0
sun_actor = None
for a in unreal.EditorLevelLibrary.get_all_level_actors():
    if a.get_component_by_class(unreal.DirectionalLightComponent):
        sun_actor = a
        break

scomp = sun_actor.get_component_by_class(unreal.DirectionalLightComponent) if sun_actor else None

for sun_val in [3.0, 5.0, 8.0, 12.0]:
    if scomp:
        scomp.set_editor_property("intensity", sun_val)
    comp.set_editor_property("post_process_blend_weight", 1.0)
    # Ensure default auto exposure bias is 0
