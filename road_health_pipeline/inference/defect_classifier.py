...
<<<<<<< HEAD
        ...
        dilated = cv2.dilate(m_u8, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15)))
        collar = (dilated > 0) & (~(m_u8 > 0))
        collar_mean = float(np.mean(gray[collar])) if np.any(collar) else (mean_val + 10.0)
        is_darker_than_collar = (collar_mean - mean_val) > 4.5

        # In low-light photography, absolute darkness threshold must be relative to collar
        is_smooth_water = std_val < 24.0
        is_relative_dark_puddle = is_darker_than_collar and std_val < 30.0

        # Check for specular reflections (clipping or localized glint)
        specular_ratio = float(np.mean((pixels[:, 0] > 230) & (pixels[:, 1] > 230) & (pixels[:, 2] > 230)))
        has_specular = specular_ratio > 0.02

        water_score = 0.0
        if is_relative_dark_puddle:
            water_score += 0.45
        if is_smooth_water and is_darker_than_collar:
            water_score += 0.35
        if has_specular:
            water_score += 0.45
