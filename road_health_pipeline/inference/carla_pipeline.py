...
def run_carla_pipeline(
    ...
    stride: int = 1,
    test_mode_2d: bool = False,
) -> Path:
    """Execute real ML detection and spatial deduplication on CARLA captured images or 2D test images."""
    ...
    log.info("Found %d image(s) in %s (TEST_MODE_2D=%s)", len(images), img_dir, test_mode_2d)

    # 2. Parse telemetry from metadata.csv if available (only enforce in 3D mode)
    ...
    if metadata_csv.is_file():
        ...
        if meta_rows and not test_mode_2d:
    ...
    for idx, img_path in enumerate(selected_images):
        ...
        if not test_mode_2d:
            ...
            # 2D Test Mode Logic: bypass 3D telemetry
            telem = Telemetry(
                timestamp=utc_iso(),
                latitude=CONFIG.carla_origin_lat,
                longitude=CONFIG.carla_origin_lon,
                altitude_m=None,
                frame_id=img_path.stem,
            )

        res = infer(img_path, pipeline=pipeline, test_mode_2d=test_mode_2d)
        ...
        if res.detections:
            ...
            for d in res.detections:
                ...
                if not test_mode_2d:
                    # 3D CARLA ray tracing
                    def_lat, def_lon, _, _ = calculate_defect_ground_gps(
                        d.bbox, w, h, telem, localizer, horizontal_fov_deg=60.0
                    )
                else:
                    # 2D test mode: assign dummy GPS based on origin
                    def_lat = CONFIG.carla_origin_lat + (d.bbox[1] / max(1, h)) * 0.0001
                    def_lon = CONFIG.carla_origin_lon + (d.bbox[0] / max(1, w)) * 0.0001
...
def main() -> None:
    ...
    parser.add_argument("--test-mode-2d", action="store_true", default=False,
                        help="Enable 2D test mode: bypass altitude, depth, and telemetry checks")
    ...
    run_carla_pipeline(
        ...
        test_mode_2d=args.test_mode_2d,
    )
